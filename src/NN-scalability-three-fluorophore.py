...
dataset_path = os.path.join('datasets', 'scalability-three-fluorophore-data.xlsx')
# Read and preprocess the data
data = pd.read_excel(dataset_path)
y_inverse = data[['ATTO425', 'FAM', 'ATTO550']]  # Corrected column names
X_inverse = data.drop(columns=['ATTO425', 'FAM', 'ATTO550'])  # Corrected column names
...
model_path = os.path.join(models_dir, 'NN-Scalability-three-fluorophore-trained-model.pth' )
torch.save(model_inverse.state_dict(), model_path)

# Save the scalers
x_scaler_path = os.path.join(models_dir, 'NN-Scalability-three-fluorophore-scaler-x-inverse.pkl' )
with open(x_scaler_path, 'wb') as file:
    pickle.dump(scaler_X_inverse, file)

y_scaler_path = os.path.join(models_dir, 'NN-Scalability-three-fluorophore-scaler-y-inverse.pkl' )
...
for i, dye in enumerate(dyes):
    ...
    if not os.path.exists(outputs_dir):
        os.makedirs(outputs_dir)
    prediction_fig_path = os.path.join(outputs_dir, f'NN-scalability-three-fluorophore-prediction-for-{dye}.svg')
    ...
    if not os.path.exists(outputs_dir):
        os.makedirs(outputs_dir)
    residuals_fig_path = os.path.join(outputs_dir, f'NN-scalability-three-fluorophore-histogramOfResiduals-for-{dye}.svg')
