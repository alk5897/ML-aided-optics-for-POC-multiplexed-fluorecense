...
y_inverse = data[['FAM', 'ATTO550', 'Cy5']]
X_inverse = data.drop(columns=['FAM', 'ATTO550', 'Cy5'])
...
X_train_inverse, X_test_inverse, y_train_inverse, y_test_inverse = train_test_split(X_normalized_inverse, y_normalized_inverse, test_size=0.20, random_state=42)
train_inverse, X_test_inverse, y_train_inverse, y_test_inverse = train_test_split(X_normalized_inverse, y_normalized_inverse, test_size=0.20, random_state=42)
...
class InverseNeuralNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.fc4(x)  # No activation on the output layer
...
dyes = ['FAM', 'ATTO550', 'Cy5']
# Define the path to the outputs directory
outputs_dir = os.path.join('outputs')
if not os.path.exists(outputs_dir):
    os.makedirs(outputs_dir)

predictions_path = os.path.join(outputs_dir, 'NN-predictions.xlsx')
mae_mse_path = os.path.join(outputs_dir, 'NN-concentrationWise-MAE&MSE.xlsx')
residuals_path = os.path.join(outputs_dir, 'NN-histogramOfResiduals.xlsx')
...
model_path = os.path.join(models_dir, 'NN-trained-model.pth' )
torch.save(model_inverse.state_dict(), model_path)

# Save the scalers
x_scaler_path = os.path.join(models_dir, 'NN_scaler_x_inverse.pkl' )
with open(x_scaler_path, 'wb') as file:
    pickle.dump(scaler_X_inverse, file)

y_scaler_path = os.path.join(models_dir, 'NN_scaler_y_inverse.pkl' )
...
for i, dye in enumerate(dyes):
    ...
    if not os.path.exists(outputs_dir):
        os.makedirs(outputs_dir)
    prediction_fig_path = os.path.join(outputs_dir, f'NN-prediction-for-{dye}.svg')
    ...
    if not os.path.exists(outputs_dir):
        os.makedirs(outputs_dir)
    residuals_fig_path = os.path.join(outputs_dir, f'NN-HistogramOfResiduals-for-{dye}.svg')
